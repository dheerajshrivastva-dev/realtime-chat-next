...
const UserChatItem: FC<UserChatItemProps> = ({friend, latestMessage, unreadMessageCount}) => {
  return (
    <div className='flex flex-1 items-center gap-x-3 text-sm font-semibold leading-6 group hover:bg-gray-800 p-2 rounded-md w-full'>
      <div className='h-8 w-8 border grid place-items-center rounded-md group-hover:border-purple-600 shrink-0 transition-all'>
      ...
      <div className='flex flex-col flex-1 overflow-hidden transition-all'>
        <span aria-hidden="true" className=' text-gray-200 group-hover:text-purple-600 truncate'>{friend?.name || ""}</span>
      ...
      {unreadMessageCount > 0 ? (
        <div className="rounded-full w-5 h-5 text-xs shrink-0 flex justify-center items-center text-white bg-purple-500 leading-3">
          {unreadMessageCount}
        </div>
      ) : <></>}
    </div>
